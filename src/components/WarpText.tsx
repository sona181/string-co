...
type BuildProps = {
  ...
  height: number;
  dpr: number;
  props: Required<Pick<WarpTextProps, "text" | "color" | "fontFamily" | "fontSize" | "fontWeight" | "letterSpacing" | "lineHeight">>
       & Pick<WarpTextProps, "strokeColor" | "strokeWidth">;
};

const buildTextCanvas = ({ container, width, height, dpr, props }: BuildProps): HTMLCanvasElement => {
  ...
  if (props.strokeColor && props.strokeWidth) {
    ...
    ctx.lineWidth   = props.strokeWidth * fit;
    ctx.lineJoin    = "round";
    ctx.lineCap     = "round";
    lines.forEach((line, i) => {
      const chars  = Array.from(line);
      let cursor   = width / 2 - measureLine(ctx, line, letterSpacing) / 2;
      chars.forEach((char, ci) => {
        ctx.strokeText(char, cursor, startY + i * lineHeight);
        cursor += ctx.measureText(char).width + (ci === chars.length - 1 ? 0 : letterSpacing);
      });
    });
  }
...
export default function WarpText({
  ...
  // Sync changed props without tearing down WebGL
  useEffect(() => {
    propsRef.current = {
      text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight,
      warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple,
