...
const RotatingTitle = dynamic(() => import("@/components/RotatingTitle"), { ssr: false });
const GlitchReveal  = dynamic(() => import("@/components/GlitchReveal"),  { ssr: false });
const GlitchLoop    = dynamic(() => import("@/components/GlitchLoop"),    { ssr: false });
const WarpText      = dynamic(() => import("@/components/WarpText"),      { ssr: false });
const GenreQuiz     = dynamic(() => import("@/components/GenreQuiz"),     { ssr: false });

export default function HomeClient({ isLoggedIn }: { readonly isLoggedIn: boolean }) {
  return (
    <div className="flex flex-col" style={{ position: "relative" }}>

      {/* ── Single Ferrofluid behind the whole page ── */}
        ...
        <section>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-32">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-20">
              {[
                { label: "01", title: "Visual customizer", body: "Pick your color, pickguard, and hardware. See the exact instrument you'll receive before you spend a dollar.", delay: 0.1 },
                { label: "02", title: "Loyalty rewards",   body: "Every order earns points. Bronze, Silver, Gold — real discounts that scale with how much you play and buy.", delay: 0.18 },
                { label: "03", title: "Verified reviews only", body: "Reviews come from customers who bought. No bots, no paid placements, no inflated stars.", delay: 0.26 },
              ].map((f) => (
